...
prFileName = 'gnss_log_oppo_open_sky_2026_03_23_21_10_28.txt';
dirName = 'demoFiles/mydata';
...
spoof.mode = 'gradual'; % TOGGLE: ['gradual' or 'instant']
spoof.delay = 0; % [s] additional delay introduced by the spoofer [s]
spoof.t_start = 15; % [epochs/s] start spoofing time
spoof.t_duration = 200; % [epochs/s] duration of the gradual drift (ignored if mode is 'instant')
spoof.target_position = [45.04913, 7.65547, 247]; % final spoofed position
...
if spoof.active
    ...
    % Generate the gradual drift trajectory BEFORE computing ranges
    [spoof] = ApplyGradualSpoof(gpsPvt_tmp, spoof);
